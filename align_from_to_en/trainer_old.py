...
import itertools
import os
import sys;sys.path.append("/data1/zhenzhang/dir2/naive_finetuning")
...
def estimate(text_model,tokenizer,image_model,image_dataloader,text_dataloader,desciption='',count_lang=None,args=None,text_delta_model=None,image_delta_model=None):
    ...
    labels = torch.Tensor(list(range(num)))
    logger.info(desciption)
    total_count_num = 0
    for l in scaled_cos:
        recall_mean = recall_k(labels,scaled_cos[l].T,[1],[1])
        logger.info(f'{l} | recall_mean:{recall_mean:.3f} | count: {l in count_lang}')
        if l in count_lang:
