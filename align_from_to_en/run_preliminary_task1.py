...
from trainer_preliminary import train, estimate
...
def get_args():
    ...
    parser.add_argument('--tuning_head', action='store_true')
    parser.add_argument('--head_lr', type=float,default=3e-6)
    ...
    parser.add_argument('--strategy', type=str,choices=['method1','method2','method3','baseline1','baseline2','preliminary'])
    parser.add_argument('--text_loss_strategy', type=str,choices=['cosine_mse','mse','contrastive','contrastive_with_image'],default='mse')
...
def prepare(args):
    ...
    args.output_dir = os.path.join(args.output_basedir,f"{args.language_list}_{args.delta_tuning}_{args.delta_type}_{args.lora_r}_{args.adapter_bottleneck_dim}_{args.soft_token_num}_{args.compacter_reduction_factor}_{args.n_shot}_t{args.text_lr}_r{args.text_loss_ratio}_{args.strategy}_{args.text_loss_strategy}_{times}")
...
if __name__=='__main__':
    ...
    ##
    # train data
    ind_image_train, ind_text_train = load_file_from_task(task_idx=1,n_obs=None,split='train', image_preprocessor=preprocess)
    _, from_text_train = load_file_from_task(task_idx=1,n_obs=None,split='train_trans_from_en', image_preprocessor=preprocess)
    _, to_text_train = load_file_from_task(task_idx=1,n_obs=None,split='train_trans_to_en', image_preprocessor=preprocess)

    # val data
    ind_image_val, ind_text_val = load_file_from_task(task_idx=1,n_obs=None,split='val', image_preprocessor=preprocess)
    _, from_text_val = load_file_from_task(task_idx=1,n_obs=None,split='val_trans_from_en', image_preprocessor=preprocess)
    _, to_text_val = load_file_from_task(task_idx=1,n_obs=None,split='val_trans_to_en', image_preprocessor=preprocess)

    # test data
    ind_image_test, ind_text_test = load_file_from_task(task_idx=1,n_obs=None,split='test_2016_flickr', image_preprocessor=preprocess)
    _, from_text_test = load_file_from_task(task_idx=1,n_obs=None,split='test_2016_flickr_trans_from_en', image_preprocessor=preprocess)
    _, to_text_test = load_file_from_task(task_idx=1,n_obs=None,split='test_2016_flickr_trans_to_en', image_preprocessor=preprocess)
    ...
    # indices = torch.randperm(len(ind_image_train), generator=generator).tolist()
    ...
    #     args.n_shot = len(ind_image_train)
    ...
    # ind_image_train, ind_image_val, ind_image_test = [ind_image_train[i] for i in indices[0:args.n_shot]], ind_image_val, ind_image_test
    # print(len(ind_image_train),len(ind_image_val),len(ind_image_test))
    # print(len(ind_text_train['en']))


    # ind_text_train_en, ind_text_val_en, ind_text_test_en = [ind_text_train['en'][i] for i in indices[0:args.n_shot]], ind_text_val['en'], ind_text_test['en']
    # ind_text_train_tgt, ind_text_val_tgt, ind_text_test_tgt = [ind_text_train[args.language][i] for i in indices[0:args.n_shot]], ind_text_val[args.language], ind_text_test[args.language]
    # from_text_train, from_text_val, from_text_test = [from_text_train[args.language][i] for i in indices[0:args.n_shot]], from_text_val[args.language], from_text_test[args.language]
    # to_text_train, to_text_val, to_text_test = [to_text_train[args.language][i] for i in indices[0:args.n_shot]], to_text_val[args.language], to_text_test[args.language]
    ...
    train_data = [ind_image_train]
    for lang in args.language_list:
        train_data.append(ind_text_train[lang])
    
    train_dataloader = torch.utils.data.DataLoader([i for i in zip(*train_data)] , batch_size=args.train_bs, shuffle=True, num_workers=0)
    for i in train_data:
        logger.info(f"train len: {len(i)}")
    ...
    for lang in ind_text_val:
        ind_texts_val_dataloader[lang] = torch.utils.data.DataLoader(ind_text_val[lang], batch_size=args.eval_bs, shuffle=False, num_workers=0)
    to_texts_val_dataloader = {}
    for lang in to_text_val:
        to_texts_val_dataloader[lang] = torch.utils.data.DataLoader(to_text_val[lang], batch_size=args.eval_bs, shuffle=False, num_workers=0)
    ...
    #     recall_overall_i =  estimate(text_model,tokenizer,image_model,images_test_dataloader,from_texts_test_dataloader,desciption=f'XTD zero-shot {args.strategy} final',count_lang=[args.language],args=args,text_delta_model=text_delta_model,image_delta_model=image_delta_model)
    # elif args.strategy=='method2' or args.strategy=='baseline1' or args.strategy=='method1':
    #     recall_overall_i =  estimate(text_model,tokenizer,image_model,images_test_dataloader,ind_texts_test_dataloader,desciption=f'XTD zero-shot {args.strategy} final',count_lang=[args.language],args=args,text_delta_model=text_delta_model,image_delta_model=image_delta_model)
    # =========================================================================
    # text model
    if args.delta_tuning:
        ...
        if args.delta_type == 'lora':
            ...
            for p in text_model.LinearTransformation.parameters():
                p.requires_grad = False
            delta_model.log(delta_ratio=True, trainable_ratio=True, visualization=True, printfile=args.output_dir)
    ...
    if args.tuning_head:
        text_unfreezed_modules_pet = [p for n,p in text_model.named_parameters() if p.requires_grad]
        logger.info([n for n,p in text_model.named_parameters() if p.requires_grad])
        text_model.transformer.pooler.requires_grad = True
        text_model.LinearTransformation.requires_grad = True
        # for p in text_model.transformer.pooler.parameters():
        #     p.requires_grad = True
        # for p in text_model.LinearTransformation.parameters():
        #     p.requires_grad = True
        text_unfreezed_modules_head = [p for n,p in text_model.named_parameters() if p.requires_grad and (("pooler" in n) or ("LinearTransformation" in n))]
        logger.info([n for n,p in text_model.named_parameters() if p.requires_grad and (("pooler" in n) or ("LinearTransformation" in n))])
        text_unfreezed_modules = [
            {'params': text_unfreezed_modules_head, 'lr': args.head_lr},
            {'params': text_unfreezed_modules_pet, 'lr': args.text_lr},
        ]
    else:    
        text_unfreezed_modules = [p for p in text_model.parameters() if p.requires_grad]

    text_unfreezed_modules_to_count = [p for p in text_model.parameters() if p.requires_grad]
    logger.info(
        f"text model: {(sum([p.numel() for p in text_unfreezed_modules_to_count])/1024**2)}/{(sum([p.numel() for p in text_model.parameters()])/1024**2)}")
    logger.info(
        f"text model radio: {(sum([p.numel() for p in text_unfreezed_modules_to_count])/1024**2)/(sum([p.numel() for p in text_model.parameters()])/1024**2)}")
    ...
    train(args=args,
        ...
        images_val_dataloader=images_val_dataloader,
        texts_val_dataloader=(to_texts_val_dataloader,ind_texts_val_dataloader),
        )
    ...
    if args.strategy=='method3' or args.strategy=='baseline2':
        recall_overall_i =  estimate(text_model,tokenizer,image_model,images_test_dataloader,to_texts_test_dataloader,desciption=f'XTD {args.n_shot}-shot {args.strategy} final',count_lang=args.language_list,args=args,text_delta_model=text_delta_model,image_delta_model=image_delta_model)
    elif args.strategy=='method2' or args.strategy=='baseline1' or args.strategy=='method1' or args.strategy=='preliminary':
        recall_overall_i =  estimate(text_model,tokenizer,image_model,images_test_dataloader,ind_texts_test_dataloader,desciption=f'XTD {args.n_shot}-shot {args.strategy} final',count_lang=args.language_list,args=args,text_delta_model=text_delta_model,image_delta_model=image_delta_model)
    ...
    if args.strategy=='method3' or args.strategy=='baseline2':
        recall_overall_i =  estimate(text_model,tokenizer,image_model,images_test_dataloader,to_texts_test_dataloader,desciption=f'XTD {args.n_shot}-shot {args.strategy} best',count_lang=args.language_list,args=args,text_delta_model=text_delta_model,image_delta_model=image_delta_model)
    elif args.strategy=='method2' or args.strategy=='baseline1' or args.strategy=='method1' or args.strategy=='preliminary':
        recall_overall_i =  estimate(text_model,tokenizer,image_model,images_test_dataloader,ind_texts_test_dataloader,desciption=f'XTD {args.n_shot}-shot {args.strategy} best',count_lang=args.language_list,args=args,text_delta_model=text_delta_model,image_delta_model=image_delta_model)
