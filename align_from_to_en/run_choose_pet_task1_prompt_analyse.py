...
def get_args():
    ...
    parser.add_argument('--strategy', type=str,choices=['method1','method2','method3','baseline1','baseline2','test_from_en'])
...
def prepare(args):
    ...
    args.output_dir = os.path.join(args.output_basedir,f"{args.language}_{args.delta_tuning}_{args.delta_type}_{args.lora_r}_{args.adapter_bottleneck_dim}_{args.soft_token_num}_{args.compacter_reduction_factor}_{args.n_shot}_t{args.text_lr}_r{args.text_loss_ratio}_{args.strategy}_{args.text_loss_strategy}_{times}")
...
if __name__=='__main__':
    ...
    from_texts_test_dataloader = {}
    for lang in from_text_test:
        from_texts_test_dataloader[lang] = torch.utils.data.DataLoader(from_text_test[lang], batch_size=args.eval_bs, shuffle=False, num_workers=0)
    # logger.info(f"test len: {len(ind_image_test)}")
    # ##
    # # independent_test data
    ...
    if args.strategy=='method3' or args.strategy=='baseline2':
        recall_overall_i =  estimate(text_model,tokenizer,image_model,images_test_dataloader,to_texts_test_dataloader,desciption=f'XTD {args.n_shot}-shot {args.strategy} final',count_lang=[args.language],args=args,text_delta_model=text_delta_model,image_delta_model=image_delta_model)
    elif args.strategy=='method2' or args.strategy=='baseline1' or args.strategy=='method1':
        recall_overall_i =  estimate(text_model,tokenizer,image_model,images_test_dataloader,ind_texts_test_dataloader,desciption=f'XTD {args.n_shot}-shot {args.strategy} final',count_lang=[args.language],args=args,text_delta_model=text_delta_model,image_delta_model=image_delta_model)
    elif args.strategy=='test_from_en':
        recall_overall_i =  estimate(text_model,tokenizer,image_model,images_test_dataloader,from_texts_test_dataloader,desciption=f'XTD {args.n_shot}-shot {args.strategy} final',count_lang=[args.language],args=args,text_delta_model=text_delta_model,image_delta_model=image_delta_model)
